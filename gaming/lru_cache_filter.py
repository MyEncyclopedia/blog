...
def lru_cache_selected2(*decorator_args, **decorator_kwargs):
    # print(*decorator_args)
    excludes = decorator_kwargs.get('excludes', [])
    decorator_kwargs.pop('excludes')
    ...
    def decorator(f):
        ...
        def filter_cached_args(*func_args):
            return [func_args[i] for i in range(len_func_args) if f_arg_spec.args[i] not in excludes]

        @lru_decorator
        def helper(box: _Equals, *effective_args, **effective_kwargs):
            # effective_kwargs.update(box.exc)
...
@lru_cache_selected2(maxsize=None, excludes=['ex1', 'ex2'])
